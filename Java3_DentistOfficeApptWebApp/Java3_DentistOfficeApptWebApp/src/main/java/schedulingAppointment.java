...
/********************************************************************
 *	Java III - Scheduling Appointment
 * 	William Weldy - Spring 2020
 *      schedulingAppointment.java
 ********************************************************************/
@WebServlet(urlPatterns = {"/schedulingAppointment"})
public class schedulingAppointment extends HttpServlet {
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            int randomDentistID = (new Random().nextInt(5))+201; //used for randomly assigning a dentist to the patient when updating the appointment later
            
            //Collect all the information needed for the servlet to process everything
            p1.selectDB(p1.getPatientID());
            apt.patientSelect(p1.getPatientID());
            proc.selectByName(procName);
            ...
            apt.insertDB(date,p1.getPatientID(),"D" + String.valueOf(randomDentistID),proc.getProcCode());
                    
            out.println("<!DOCTYPE html>");
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Scheduling Appointment</title>");            
            out.println("</head>");
            out.println("<body>");
            
                out.println("<h2>Your " + procName + " appointment is scheduled for " + date + ".</h2> <a href=\"Patient_main.jsp\"> return to the main page.");
    ...
    @Override
    public String getServletInfo() { return "this will take all the information from Patient_changeAppointment.jsp and run the code necessary to make changes to the Appointments table."; }
}
